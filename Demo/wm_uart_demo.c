...
static void pin_function_init(void)
{
	u32 regValue;
	...
	//打开 PA0 PA1 PA2 PA3的复用功能
	regValue = tls_reg_read32(HR_GPIOA_AFSEL);
	regValue |= 0x0F;
	tls_reg_write32(HR_GPIOA_AFSEL, regValue);

	//选择要用到的具体的复用功能，此处选择opt1
	regValue = tls_reg_read32(HR_GPIOA_AFS0);
	regValue &= ~0x0F;
	tls_reg_write32(HR_GPIOA_AFS0, regValue);
    
	regValue = tls_reg_read32(HR_GPIOA_AFS1);
	regValue &= ~0x0F;
	tls_reg_write32(HR_GPIOA_AFS1, regValue);

#define BIT(x) (1UL << (x))

    //将接收引脚PA0上拉，防止因无外部上拉时设备持续进入接收中断函数
    tls_reg_write32(HR_GPIOA_DATA_PULLEN, tls_reg_read32(HR_GPIOA_DATA_PULLEN) & (~ BIT(WM_IO_PA_00)));
}
        ...
        switch ((u32) msg)
        {
            case DEMO_MSG_OPEN_UART:
				{				
					...
					tls_uart_rx_callback_register((u16) TLS_UART_2, demo_uart_rx);
					tls_uart_tx_callback_register(TLS_UART_2, uart_tx_sent_callback);
            	}
                break;
            ...
            case DEMO_MSG_UART_RECEIVE_DATA:
				{
	                ...
	                while (rx_len > 0)
	                {
	                    ...
	                    tls_uart_write(TLS_UART_2, uart->rx_buf, len);  /* output */
						//tls_uart_dma_write(uart->rx_buf, len, uart_dma_done, TLS_UART_2);
	                }
