...
u8 exceptchan = 0xFF;
#define TLS_ONESHOT_RESTART_TIME  3000*HZ/1000
#define TLS_ONESHOT_RETRY_TIME   5000*HZ/1000
#define TLS_ONESHOT_RECV_TIME   15000*HZ/1000
#define TLS_ONESHOT_SWITCH_TIMER_MAX (50*HZ/1000)
...
void tls_oneshot_task_handle(void *arg)
{
    ...
    for(;;)
    {
        ...
        switch((u32)msg)
        {
            ...
            if (exceptchan == 0xFF)
            {
			    wifi_change_chanel(airwifichan[chanCnt], bandwidth);
            }
            else{
			    wifi_change_chanel(airwifichan[chanCnt], exceptchan);		   	
            }
            ...
            if (chanCnt >= uctotalchannum)
            {
                ...
                if (exceptchan == 0xFF)
                {
                    ...
                    bandwidth = exceptchan;
                }
            }
            wifi_change_chanel(airwifichan[chanCnt], bandwidth);
            ONESHOT_DBG("chan:%d,bandwidth:%d\n", airwifichan[chanCnt], bandwidth);
#if TLS_CONFIG_AIRKISS_MODE_ONESHOT
            ...
            case ONESHOT_HANDSHAKE_TIMEOUT:
                exceptchan = bandwidth^1;
                ONESHOT_DBG("chwidth:%d, %d\n", exceptchan, bandwidth);
